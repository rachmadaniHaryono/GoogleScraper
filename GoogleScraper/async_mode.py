    ...
    async def __call__(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                logger.info(
                    "[+] {} requested keyword '{}' on {}. Response status: {}".format(
                        self.requested_by,
                        self.query,
                ...
                logger.debug("[i] URL: {} HEADERS: {}".format(url, self.headers))
...
class AsyncScrapeScheduler(object):
    ...
    def run(self):
        ...
        while True:
            ...
            self.results = self.loop.run_until_complete(
                asyncio.wait([r() for r in self.requests])
            )
