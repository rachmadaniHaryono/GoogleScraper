...
def check_detection(config, search_engine_name):
    ...
    if search_engine_name == "google":
        ...
        def check(browser, status):
            ...
            if (
                needles["inurl"] in browser.current_url
                and needles["inhtml"] in browser.page_source
            ):
...
def get_selenium_scraper_by_search_engine_name(
    config, search_engine_name, *args, **kwargs
):
    ...
    """
    class_name = (
        search_engine_name[0].upper() + search_engine_name[1:].lower() + "SelScrape"
    )
    ns = globals()
    if class_name in ns:
...
class SelScrape(SearchEngineScrape, threading.Thread):
    ...
    def proxy_check(self, proxy):
        assert (
            self.proxy and self.webdriver
        ), "Scraper instance needs valid webdriver and proxy instance to make the proxy check"

        online = False
        status = (
            "Proxy check failed: {host}:{port} is not used while requesting".format(
                **self.proxy.__dict__
            )
        )
        ipinfo = {}
        ...
        try:
            ...
            try:
                text = re.search(
                    r"(\{.*?\})", self.webdriver.page_source, flags=re.DOTALL
                ).group(0)
    ...
    def _save_debug_screenshot(self):
        ...
        location = os.path.join(
            tempdir,
            "{}_{}_debug_screenshot.png".format(
                self.search_engine_name, self.browser_type
            ),
        )
    ...
    def _get_Chrome(self):
        try:
            ...
            if self.proxy:
                chrome_options.add_argument(
                    "--proxy-server={}://{}:{}".format(
                        self.proxy.proto, self.proxy.host, self.proxy.port
                    )
                )
    ...
    def handle_request_denied(self):
        ...
        if self.malicious_request_detected():
            ...
            if (
                self.config.get("manual_captcha_solving", False)
                and self.config.get("browser_mode") != "headless"
            ):
    ...
    def _wait_until_search_input_field_appears(self, max_wait=5):
        ...
        try:
            search_input = WebDriverWait(self.webdriver, max_wait).until(
                find_visible_search_input
            )
            return search_input
        except TimeoutException as e:
            logger.error(
                "{}: TimeoutException waiting for search input field: {}".format(
                    self.name, e
                )
            )
            return False
    ...
    def _wait_until_search_param_fields_appears(self, max_wait=5):
        ...
        try:
            fields = WebDriverWait(self.webdriver, max_wait).until(
                find_visible_search_param
            )
            return fields
        except TimeoutException as e:
            logger.error(
                "{}: TimeoutException waiting for search param field: {}".format(
                    self.name, e
                )
            )
            return False
    ...
    def _find_next_page_element(self):
        ...
        if self.search_type == "normal":
            ...
            try:
                ...
                logger.warning(
                    "{}: Cannot locate next page element: {}".format(self.name, str(e))
                )
                return False
                ...
                try:
                    WebDriverWait(self.webdriver, 5).until(
                        EC.text_to_be_present_in_element(
                            (By.CSS_SELECTOR, selector), str(self.page_number)
                        )
                    )
                except TimeoutException as e:
                    self._save_debug_screenshot()
                    logger.warning(
                        "Pagenumber={} did not appear in serp. Maybe there is only one result for this query?".format(
                            self.page_number
                        )
                    )
    ...
    def build_search(self):
        ...
        if self.search_engine_name == "google":
            if num_results not in (10, 20, 30, 50, 100):
                raise Exception(
                    "num_results_per_page for selenium mode and search engine Google must be in (10, 20, 30, 50, 100)"
                )
            ...
            if num_results not in range(1, 100):
                raise Exception(
                    "num_results_per_page for selenium mode and search engine Bing must be in range(1, 100)"
                )
            ...
            if num_results not in range(1, 100):
                raise Exception(
                    "num_results_per_page for selenium mode and search engine Yahoo must be in range(1, 100)"
                )
            ...
            if self.search_input:
                ...
                if self.search_param_fields:
                    ...
                    if wait_res is False:
                        raise Exception(
                            "Waiting search param input fields time exceeds"
                        )
                ...
                logger.debug(
                    "{}: Cannot get handle to the input form for keyword {}.".format(
    ...
    def run(self):
        ...
        if not self._get_webdriver():
            raise Exception(
                "{}: Aborting: No available selenium webdriver.".format(self.name)
            )

        try:
            self.webdriver.set_window_size(400, 400)
            self.webdriver.set_window_position(
                400 * (self.browser_num % 4), 400 * (math.floor(self.browser_num // 4))
            )
        except WebDriverException as e:
...
class GoogleSelScrape(SelScrape):
    ...
    def build_search(self):
        ...
        if self.config.get("google_selenium_search_settings", False):
            ...
            element = WebDriverWait(self.webdriver, 7).until(
                EC.presence_of_element_located((By.NAME, "safeui"))
            )

            try:
                if self.config.get("google_selenium_safe_search", False):
                    if (
                        self.webdriver.find_element_by_name("safeui").get_attribute(
                            "value"
                        )
                        != "on"
                    ):
                ...
                try:
                    if self.config.get("google_selenium_personalization", False):
                        self.webdriver.find_element_by_css_selector(
                            "#pson-radio > div:first-child"
                        ).click()
                    else:
                        self.webdriver.find_element_by_css_selector(
                            "#pson-radio > div:nth-child(2)"
                        ).click()
                except WebDriverException as e:
                ...
                try:
                    ...
                    # reset
                    for i in range(5):
                        self.webdriver.find_element_by_id("result_slider").send_keys(
                            Keys.LEFT
                        )
...
class AskSelScrape(SelScrape):
    ...
    def wait_until_serp_loaded(self):
        def wait_until_keyword_in_url(driver):
            try:
                return (
                    quote(self.query) in driver.current_url
                    or self.query.replace(" ", "+") in driver.current_url
                )
            except WebDriverException:
                pass
