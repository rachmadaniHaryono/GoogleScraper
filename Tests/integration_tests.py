...
class GoogleScraperIntegrationTestCase(
            ...
            for key, value in c.items():
                if key not in exclude_keys:
                    assert (len(res) / int(value)) >= 9, (
                        key
                        + " has too many times a None value: "
                        + "{}/{}".format(int(value), len(res))
                    )

    def test_parse_google(self):
        parser = self.get_parser_for_file(
            "google", "data/uncompressed_serp_pages/abrakadabra_google_de_ip.html"
        )
    ...
    def test_parse_yahoo(self):
        ...
        assert (
            ...
            == 10
        ), "Not 10 elements with a visible link in yahoo serp page"
        assert all([v["link"] for v in parser.search_results["results"]])
        self.assert_around_10_results_with_snippets(parser)
        assert any(
            [
    ...
    def test_parse_yandex(self):
        ...
        assert (
            ...
            == 10
        ), "Not 10 elements with a visible link in yandex serp page"
        assert all([v["link"] for v in parser.search_results["results"]])
        self.assert_around_10_results_with_snippets(parser)
        assert any(
            [
                "n play games to compile games statist" in v["snippet"]
                for v in parser.search_results["results"]
                if v["snippet"]
            ]
        ), "Specific string not found in snippet."
    ...
    def test_parse_baidu(self):
        ...
        parser = self.get_parser_for_file(
            "baidu", "data/uncompressed_serp_pages/number_baidu_de_ip.html"
        )

        assert "100,000,000" in parser.num_results_for_query
        assert len(parser.search_results["results"]) >= 6, len(
            parser.search_results["results"]
        )
        assert all([v["link"] for v in parser.search_results["results"]])
    ...
    def test_parse_duckduckgo(self):
        ...
        parser = self.get_parser_for_file(
            "duckduckgo", "data/uncompressed_serp_pages/mountain_duckduckgo_de_ip.html"
        )
    ...
    def test_parse_ask(self):
        ...
        parser = self.get_parser_for_file(
            "ask", "data/uncompressed_serp_pages/fellow_ask_de_ip.html"
        )
        ...
        assert (
            ...
            == 10
        ), "Not 10 elements with a visible link in ask serp page"
        assert all([v["link"] for v in parser.search_results["results"]])
        self.assert_around_10_results_with_snippets(parser)
        self.assert_atleast90percent_of_items_are_not_None(parser)
        ...
        for rownum, row in enumerate(reader):
            if rownum == 0:
                ...
                assert header_keys.issubset(
                    set(csv_fieldnames)
                ), "Invalid CSV header: {}".format(header)

            for item in notnull:
                assert row[
                    header.index(item)
                ], "{} has a item that has no value: {}".format(item, row)
    ...
    def test_json_output_static(self):
        ...
        try:
            ...
            print(
                "Cannot parse output json file {}. Reason: {}".format(json_outfile, e)
            )
            raise e
    ...
    def test_no_results_for_query_google(self):
        parser = self.get_parser_for_file(
            "google", "data/uncompressed_no_results_serp_pages/google.html"
        )

        assert (
            parser.effective_query == '"be dealt and be evaluated"'
        ), "No effective query."

    def test_no_results_for_query_yandex(self):
        parser = self.get_parser_for_file(
            "yandex", "data/uncompressed_no_results_serp_pages/yandex.html"
        )

        assert parser.effective_query == "food", "Wrong effective query. {}".format(
            parser.effective_query
        )

    def test_no_results_for_query_bing(self):
        parser = self.get_parser_for_file(
            "bing", "data/uncompressed_no_results_serp_pages/bing.html"
        )
    ...
    def test_no_results_for_query_ask(self):
        parser = self.get_parser_for_file(
            "ask", "data/uncompressed_no_results_serp_pages/ask.html"
        )
    ...
    def test_page_number_selector_yandex(self):
        parser = self.get_parser_for_file(
            "yandex", "data/page_number_selector/yandex_5.html"
        )
        assert parser.page_number == 5, "Wrong page number. Got {}".format(
            parser.page_number
        )
    ...
    def test_page_number_selector_google(self):
        ...
        parser = self.get_parser_for_file(
            "google", "data/page_number_selector/google_8.html"
        )
        assert parser.page_number == 1, "Wrong page number. Got {}".format(
            parser.page_number
        )

    def test_page_number_selector_bing(self):
        parser = self.get_parser_for_file(
            "bing", "data/page_number_selector/bing_5.html"
        )
        assert parser.page_number == 5, "Wrong page number. Got {}".format(
            parser.page_number
        )
    ...
    def test_page_number_selector_yahoo(self):
        parser = self.get_parser_for_file(
            "yahoo", "data/page_number_selector/yahoo_3.html"
        )
        assert parser.page_number == 3, "Wrong page number. Got {}".format(
            parser.page_number
        )

    def test_page_number_selector_baidu(self):
        parser = self.get_parser_for_file(
            "baidu", "data/page_number_selector/baidu_9.html"
        )
        assert parser.page_number == 9, "Wrong page number. Got {}".format(
            parser.page_number
        )

    def test_page_number_selector_ask(self):
        parser = self.get_parser_for_file("ask", "data/page_number_selector/ask_7.html")
        assert parser.page_number == 7, "Wrong page number. Got {}".format(
            parser.page_number
        )
    ...
    def test_no_results_serp_object(self):  # pylint: disable=no-self-use
        ...
        assert len(all_search_engines) == len(
            search.serps
        ), "Not enough results. Expected: {}, got {}".format(
        ...
        for serp in search.serps:
            ...
            if serp.search_engine_name in ("google", "bing"):
                ...
                ), "{} must have an effective query when a keyword has no results.".format(
    ...
    def test_no_results2_static(self):
        ...
        for search_engine in ("google", "duckduckgo", "bing", "yahoo"):
            parser = self.get_parser_for_file(
                search_engine,
                "data/no_results_literal/{}.html".format(search_engine),
                query=query,
            )
    ...
    def test_duckduckgo_http_mode_works(self):
        ...
        parser = self.get_parser_for_file(
            "duckduckgo",
            "data/various/duckduckgo_http_mode_december_2015.html",
            query="what happened",
        )
